...
## - CAPM cost of equity (normal + zero-intercept)
...
def compute_betas(df: pd.DataFrame, stock_col: str, mkt_col: str, rf_col: str) -> Dict[str, object]:
    ...
    compute:
      ...
      - zero-intercept beta (simple)
      - excess-return beta (with intercept, CAPM-style)
      - excess-return beta (zero-intercept CAPM-style)
    ...
    # ---------- Excess-return beta (CAPM-style, with intercept) ----------
    ...
    # ---------- Excess-return beta (CAPM-style, zero-intercept) ----------
    X_ex0 = df[["excess_mkt"]]  # no constant
    reg_excess_zero = sm.OLS(y_ex, X_ex0).fit()
    beta_excess_zero = reg_excess_zero.params["excess_mkt"]
...
def plot_regression(
    ...
    """
    Scatter plot of returns with regression line (with intercept).
    """
    x = df[x_col]
    y = df[y_col]
...
def run_beta_engine(
    ...
    """
    Pulls data, computes betas, CAPM cost of equity (normal & zero-intercept),
    writes out an Excel file, and saves regression plots to 'graphs/'.
    """
    check_fred_api_key()
    ...
    beta_excess_zero = results["beta_excess_zero"]
    ...
    print("\nExcess-return regression (CAPM-style, with intercept):")
    print(f"  (R_i - R_f) = {alpha_excess:.6f} + {beta_excess:.4f} * (R_m - R_f)")
    print(f"  Beta_excess = {beta_excess:.4f}")

    print("\nExcess-return regression (CAPM-style, zero-intercept):")
    print(f"  (R_i - R_f) = {beta_excess_zero:.4f} * (R_m - R_f)")
    print(f"  Beta_excess_zero = {beta_excess_zero:.4f}")
    print("======================================================\n")
    ...
    cost_of_equity_excess_zero = capm_cost_of_capital(beta_excess_zero)

    print(f"CAPM cost of equity (normal simple beta):        {cost_of_equity_normal:.4%}")
    print(f"CAPM cost of equity (zero-intercept simple beta): {cost_of_equity_zero:.4%}")
    print(f"CAPM cost of equity (excess-return beta):         {cost_of_equity_excess:.4%}")
    print(f"CAPM cost of equity (excess-return zero beta):    {cost_of_equity_excess_zero:.4%}")
    ...
    summary = pd.DataFrame(
        {
            ...
            "cost_of_capital_excess_zero": [cost_of_equity_excess_zero],
        }
    )
    ...
    # -----------------------------
    # Plots: all four versions, saved to graphs/
    # -----------------------------
    base_graph_name = f"{safe_stock}_{safe_mkt}_{rf_series}"

    # 1) Simple returns, normal regression
    plot_regression(
        data,
        x_col="mkt_ret",
        y_col="stock_ret",
        alpha=alpha_normal,
        beta=beta_normal,
        title=f"{stock_ticker} vs {market_ticker} (Normal Regression, Simple Returns)",
        save_path=f"graphs/{base_graph_name}_simple_normal.png",
    )

    # 2) Simple returns, zero-intercept
    plot_regression_zero(
        data,
        x_col="mkt_ret",
        y_col="stock_ret",
        beta=beta_zero,
        title=f"{stock_ticker} vs {market_ticker} (Zero-Intercept, Simple Returns)",
        save_path=f"graphs/{base_graph_name}_simple_zero.png",
    )

    # 3) Excess returns, normal (with intercept)
    plot_regression(
        data,
        x_col="excess_mkt",
        y_col="excess_stock",
        alpha=alpha_excess,
        beta=beta_excess,
        title=f"{stock_ticker} vs {market_ticker} (Normal Regression, Excess Returns)",
        save_path=f"graphs/{base_graph_name}_excess_normal.png",
    )

    # 4) Excess returns, zero-intercept
    plot_regression_zero(
        data,
        x_col="excess_mkt",
        y_col="excess_stock",
        beta=beta_excess_zero,
        title=f"{stock_ticker} vs {market_ticker} (Zero-Intercept, Excess Returns)",
        save_path=f"graphs/{base_graph_name}_excess_zero.png",
    )
    ...
    return {
        ...
        "cost_of_equity_normal": cost_of_equity_normal,
        "cost_of_equity_zero": cost_of_equity_zero,
        "cost_of_equity_excess": cost_of_equity_excess,
        "cost_of_equity_excess_zero": cost_of_equity_excess_zero,
    }
...
if __name__ == "__main__":
    ...
    k_e = beta_results["cost_of_equity_excess"]  # still using excess-return beta as primary
    ...
    annual_rate = get_float_input(
        "Annual interest rate (e.g., 0.05 for 5%) [0.05]: ",
        default=0.05,
        min_val=-1.0,
    )
